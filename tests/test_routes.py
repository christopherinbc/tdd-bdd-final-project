...
class TestProductRoutes(TestCase):
    ...
    def test_get_product(self):
        ...
        # make a self.client.get request to the API endpoint and store the
        # result in the variable named response
    ...
    def test_get_product_not_found(self):
        ...
        # send a self.client.get() request to the BASE_URL with an invalid
        # product ID (e.g., 0)
    ...
    def test_update_product(self):
        ...
        test_product = ProductFactory()
        # send a self.client.post() request to the BASE_URL with a json payload of
    ...
    def test_delete_product(self):
        ...
        # send a self.client.get request to the same endpoint that was deleted to retrieve the
        # deteled product
        response = self.client.get(f"{BASE_URL}/{test_product.id}")
        # assert that the resp.status_code is status.HTTP_404_NOT_FOUND to confirm deletion of
        # the product
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_query_by_name(self):
        ...
        products = self._create_products(5)
        # extract the name of the first product in the products list and assigns it to the
        # variable test_name
        test_name = products[0].name
        # count the number of products in the products list that have the same name as the
        # test_name
        name_count = len([product for product in products if product.name == test_name])
        # send an HTTP GET request to the URL specified by the BASE_URL variable, along with
        ...
        data = response.get_json()
        # assert that the length of the data list (i.e., the number of products returned in the
        # response) is equal to name_count
        self.assertEqual(len(data), name_count)
        # use a for loop to iterate through the products in the data list and checks if each
        # product's name matches the test_name
        for product in data:
    ...
    def test_query_by_category(self):
        ...
        # retrieves the category of the first product in the products list and assigns it to the
        # variable category
        category = products[0].category
        # create a list named found, containing products from the products list whose category
        # matches the category variable
        found = [product for product in products if product.category == category]
        # check the count of products match the specified category and assign it to the variable
        # found_count
        ...
        # send an HTTP GET request to the URL specified by the BASE_URL variable, along with a
        ...
        data = response.get_json()
        # assert that the length of the data list (i.e., the number of products returned in the
        # response) is equal to found_count
        self.assertEqual(len(data), found_count)
        # use a for loop to check each product in the data list and verify that all returned
        # products belong to the queried category
        for product in data:
    ...
    def test_query_by_availability(self):
        ...
        products = self._create_products(10)
        # list named available_products is initialized to store the products based on their
        # availability status
        ...
        # send an HTTP GET request to the URL specified by the BASE_URL variable, along with
        # a query parameter "available" set to true.
        ...
        data = response.get_json()
        # assert that the length of the data list (i.e., the number of products returned in the
        # response) is equal to available_count
        self.assertEqual(len(data), available_count)
        # use a for loop to check each product in the data list and verify that the "available"
        # attribute of each product is set to True
        for product in data:
